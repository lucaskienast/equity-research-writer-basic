...
- **LangChain**
- **LangGraph**
- **OpenAI or Anthropic API** (provider-selectable via `LLM_PROVIDER`)
- **Flask web UI**
- **Azure Blob Storage**
...
flowchart TD
    ...
    B --> C[Load settings and initialise LLM client]
...
- model name, temperature, max tokens (shared across providers)
- OpenAI API key
- Anthropic API key
- Azure connection string, container name, blob prefix
...
`ResearchClient` wraps either `ChatOpenAI` or `ChatAnthropic` depending on `LLM_PROVIDER`, and ensures every generation call uses:
...
- `GET /` — dark/light mode UI with file upload
- `POST /api/run` — starts the workflow in a background thread, returns a `job_id`
- `GET /api/status/<job_id>` — poll for `running` / `done` / `error` and retrieve results

Start with `make run-web` or `python -m equity_research_agent.web` — listens on http://localhost:5000.
...
- reads input text from `--text`, `--input-file`, or stdin (PDF and TXT supported)
- accepts `--company`, `--ticker`, `--analyst` metadata flags
- runs the graph
- prints the final markdown to the console
- writes `analyst_review.md`, `morning_note.md`, and `research_note.json` locally
- uploads to Azure if `--upload` or `UPLOAD_TO_AZURE=true`
...
### Step 6: Wrap the LLM in a reusable client

Create one class (`ResearchClient`) that:

- initialises `ChatOpenAI` or `ChatAnthropic` based on `LLM_PROVIDER`
...
- upload all three artifact files
- use a blob path convention such as:

```text
equity-research/YYYY/MM/DD/run-id/analyst_review.md
```
...
At minimum, for local generation you need one of:
...
- File upload (PDF or TXT, up to 20 MB) with optional inline text note
- Company, ticker, and analyst metadata fields
- Async job submission — UI polls until complete
- Two-panel output showing analyst review and morning note side by side
